...
int CTrade::OpenPendingOrder(string pSymbol,int pType,double pVolume,double pPrice,double pStop,double pProfit,string pComment,datetime pExpiration,color pArrow)
{
   int retryCount = 0;
	...
	{
		while(IsTradeContextBusy()) Sleep(10);
		//Print("Sending ", pSymbol, " " ,  pType, " " , pVolume, " ", pPrice, " " , pStop, " " , pProfit);
		ticket = OrderSend(pSymbol, pType, pVolume, pPrice, _slippage, 0, 0, pComment, _magicNumber, pExpiration, pArrow);
   	...
   	if(ticket == -1)
   	{
      	...
      	if(checkError == false)  
   		{
     			Print("Open ",orderType," order: Error ",errorCode," - ",errDesc);
...
bool CTrade::CloseMultipleOrders(CLOSE_MARKET_TYPE pCloseType)
{
 ...
 bool CTrade::CloseExpiredOrders(int BarNb){
   bool error = false;
   bool closeOrder = false;
...
bool ModifyStopsByPoints(int pTicket, int pStopPoints, int pProfitPoints = 0, int pMinPoints = 10)
{
   ...
   if (pMinPoints<minStopLevel)pMinPoints= minStopLevel;
