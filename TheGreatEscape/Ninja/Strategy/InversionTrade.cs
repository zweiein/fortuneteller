...
namespace NinjaTrader.Strategy
{
    ...
    public class InversionTrade : Strategy
    {
        #region Variables
		
		
		private int		target1			= 15;
private int atrPeriods=1;
		private int lots=1;
        // Wizard generated variables
        private int maLen = 10; // Default setting for RSI_Period
		private int ticksDiff=0;
		private bool useInversion=true;
		private bool useHighLowEntry=false;
		private int entryTrendSignal=2;
		private int SL = 10; // Default setting for RSI_Period
...
//
		...
		*/	
        ...
        protected override void OnBarUpdate()
        {
			...
			if (CurrentBar < 20)
				return;
			double atrv=ATR(Close,atrPeriods)[0];
			this.SetStopLoss(CalculationMode.Ticks, atrv*SL);
			
			this.SetProfitTarget(CalculationMode.Ticks,target1*atrv);
			...
			if (x==ticksDiff) if (useInversion)EnterShort();else EnterLong();
			if (y==ticksDiff) if (useInversion) EnterLong();else EnterShort();
		...
		[Description("lots")]
        [GridCategory("Parameters")]
        public bool UseInversion
		{
            get { return useInversion; }
            set { useInversion = value; }
