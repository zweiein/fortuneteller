...
namespace NinjaTrader.Strategy
{
    ...
    /// </summary>
    [Description("A strategy that pyramids orders in the same direction")]
    public class RenkoPyramiding : Strategy
    {
        #region Variables
        ...
        private bool noHistorical=true;   // Set to false for backtesting
        private bool noEntryOnImmediateBars=false;   // If bar follows another bar <0.1 seconds, skip
        private bool ignoreBuiltInErrorHandling=false;
        private int maxContracts = 3;
        ...
        // A map holding submitted/accepted stop orders, so that these can be removed when new orders are entered
        ...
        protected override void OnExecution(IExecution execution)
        {
            ...
            if (execution.Order.OrderType==OrderType.Market && IsFill(execution.Order)) {   
                ...
                if  (execution.Order.OrderState == OrderState.Filled){//} && execution.Order.OrderAction!=OrderAction.BuyToCover && execution.Order.OrderAction!=OrderAction.Sell){
                    RemoveAllStopOrders();  
                    UpdateTrailing();                   
                }                
            }                   
        ...
        {
            int qua = Math.Abs(Position.Quantity); //Quantity to exit
            RemoveAllStopOrders();
            if (netPos>0) {
        ...
        private void SyncLocalVariables(){
            if (Position.MarketPosition==MarketPosition.Long){
                ...
                netPos=-Math.Abs(Position.Quantity);
                sentPos=-Math.Abs(Position.Quantity);
            }else{
                sentPos=0;
                netPos=0;
            }   
        }
        private bool OverfillDetectedAndAdjusted(){
            bool flag=false;
            if (Position.MarketPosition==MarketPosition.Long && Math.Abs(Position.Quantity)>maxContracts){
                SyncLocalVariables();
                SubmitOrder(0,OrderAction.Sell, OrderType.Market, (Math.Abs(Position.Quantity)-maxContracts), 0, 0, "", "XitL");
                flag=true;
            }           
            else if (Position.MarketPosition==MarketPosition.Short && Math.Abs(Position.Quantity)>maxContracts){
                SyncLocalVariables();
                SubmitOrder(0,OrderAction.BuyToCover, OrderType.Market, (Math.Abs(Position.Quantity)-maxContracts), 0, 0, "", "XitS");              
                flag=true;
            }
            return flag;
        }
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (sec>=1){
                SyncLocalVariables();
                if (OverfillDetectedAndAdjusted()) return;
                DisplayStatusBox();
            }
            UpdateTrailing();
            if (NoEntryOnImmediateBars && sec<0.1) return;// Dont enter on immediate bars
        ...
        [Description("")]
        [GridCategory("Parameters")]
        public bool NoEntryOnImmediateBars
        {
            get { return noEntryOnImmediateBars; }
            set { noEntryOnImmediateBars = value; }
        }        
        [Description("")]
        [GridCategory("Parameters")]
