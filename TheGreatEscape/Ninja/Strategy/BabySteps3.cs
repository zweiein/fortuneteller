...
namespace NinjaTrader.Strategy
{
    ...
    public class BabySteps3 : Strategy
    {
        #region Variables
        ...
        protected override void OnBarUpdate()
        {
			...
			ManageOrders(0,0); 
			ManageOrders(1,0);
			if (MarketPosition.Flat==m_OrderManager.GetMarketPosition(0) && MarketPosition.Flat==m_OrderManager.GetMarketPosition(1)){							
					ManageOrders(0,direction);
			}
			else if (MarketPosition.Flat==m_OrderManager.GetMarketPosition(1)){							

				double price=Close[0];
				if (MarketPosition.Long==m_OrderManager.GetMarketPosition(0) && m_OrderManager.GetAvePrice(0)<price && m_OrderManager.GetBarOfEntry(0)<CurrentBar)
					ManageOrders(1,-1); 
				else if (MarketPosition.Short==m_OrderManager.GetMarketPosition(0) && m_OrderManager.GetAvePrice(0)>price  && m_OrderManager.GetBarOfEntry(0)<CurrentBar)
					ManageOrders(1,1); 				
				
			}else if (MarketPosition.Flat==m_OrderManager.GetMarketPosition(0)){							

				double price=Close[0];
				if (MarketPosition.Long==m_OrderManager.GetMarketPosition(1) && m_OrderManager.GetAvePrice(1)<price && m_OrderManager.GetBarOfEntry(1)<CurrentBar)
					ManageOrders(0,-1); 
				else if (MarketPosition.Short==m_OrderManager.GetMarketPosition(1) && m_OrderManager.GetAvePrice(1)>price  && m_OrderManager.GetBarOfEntry(1)<CurrentBar)
					ManageOrders(0,1); 				
		...
		private void EnterShortLim(int position){
			...
			if (this.GetCurrentBid()>0 && price<this.GetCurrentBid())
			 ...
			 m_OrderManager.GoShortLimit((int)lots,(double)price,(int)position) ; 
		...
		private void EnterLongLim(int position){
			...
			m_OrderManager.GoLongLimit((int)lots,(double)price,(int)position) ; // Place Limit Order at "m_Limit"
