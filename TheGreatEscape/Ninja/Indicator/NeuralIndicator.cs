...
namespace NinjaTrader.Indicator
{
    ...
    public class NeuralIndicator : Indicator
    {
        #region Variables
		...
		protected void GotPacket(String line) {	
			...
			else if ( string.Compare(list[0],PACKET_IND,true)==0 )
			{
				if( list.Count < 8 )
				{
					...
					_indicatorData[0] = double.NaN;
					_indicatorData[1] = double.NaN;
					_indicatorData[2] = double.NaN;
					return;
				}
				...
				try
				{
					for(int i=3;i<6;i++)
					{
						Log(i + " " + list[i+1], LogLevel.Information);
						if( "?".Equals(list[i+1]) )
						{
							_indicatorData[i-3] = double.NaN;
						}
						else
						{
							_indicatorData[i-3] = double.Parse(list[i+1],_cultureUSA);
						}
					}
				}
				...
				if( index!=-1 )
				{
					...
					if( index2!=-1) 
					{
						if (index2==index+1){
							var result = new List<string>();	
							result.Add("");
							indicatorParams=new List<string>();	
						}else{
        ...
        protected override void Initialize()
        {
			...
			Add(new Plot(Color.FromKnownColor(KnownColor.Green), PlotStyle.Line, "Bar1"));		
            Overlay				= false;
...
namespace NinjaTrader.Indicator
{
    ...
    {
        private NeuralIndicator[] cacheNeuralIndicator = null;

        private static NeuralIndicator checkNeuralIndicator = new NeuralIndicator();

        /// <summary>
        /// (Version Beta-1) This is a generic indicator used to connect to the Encog Framework(www.encog.org).
        /// </summary>
        /// <returns></returns>
        public NeuralIndicator NeuralIndicator(string host, string indicatorName, int port)
        {
            return NeuralIndicator(Input, host, indicatorName, port);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public NeuralIndicator NeuralIndicator(Data.IDataSeries input, string host, string indicatorName, int port)
        {
            if (cacheNeuralIndicator != null)
                for (int idx = 0; idx < cacheNeuralIndicator.Length; idx++)
                    if (cacheNeuralIndicator[idx].Host == host && cacheNeuralIndicator[idx].IndicatorName == indicatorName && cacheNeuralIndicator[idx].Port == port && cacheNeuralIndicator[idx].EqualsInput(input))
                        return cacheNeuralIndicator[idx];

            lock (checkNeuralIndicator)
            {
                checkNeuralIndicator.Host = host;
                host = checkNeuralIndicator.Host;
                checkNeuralIndicator.IndicatorName = indicatorName;
                indicatorName = checkNeuralIndicator.IndicatorName;
                checkNeuralIndicator.Port = port;
                port = checkNeuralIndicator.Port;
                ...
                NeuralIndicator indicator = new NeuralIndicator();
                ...
                NeuralIndicator[] tmp = new NeuralIndicator[cacheNeuralIndicator == null ? 1 : cacheNeuralIndicator.Length + 1];
                if (cacheNeuralIndicator != null)
                    cacheNeuralIndicator.CopyTo(tmp, 0);
                tmp[tmp.Length - 1] = indicator;
                cacheNeuralIndicator = tmp;
                return indicator;
            }
        }
    }
...
namespace NinjaTrader.MarketAnalyzer
{
    public partial class Column : ColumnBase
    {
        ...
        public Indicator.NeuralIndicator NeuralIndicator(string host, string indicatorName, int port)
        {
            return _indicator.NeuralIndicator(Input, host, indicatorName, port);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public Indicator.NeuralIndicator NeuralIndicator(Data.IDataSeries input, string host, string indicatorName, int port)
        {
            return _indicator.NeuralIndicator(input, host, indicatorName, port);
        }
    }
