...
class TicksTotalResult(tickSize: Double, calibration: List[CalibratedIndicator]) extends OutputTarget with fortuneteller.indicators.IndicatorConfig {
  name = "TicksTotalResult"
  version="1"
  val cat = org.apache.log4j.Category.getInstance("fortuneteller.ticktotresult");
  val field = getNormalization(lookupCalibration("output.ticktotresult", calibration)); //new NormalizedField(NormalizationAction.Normalize,"diff",Config.getConfig.DONCHIAN_WIDTH_MAX,Config.getConfig.DONCHIAN_WIDTH_MIN,1,0);
  ...
  override def calibration(window: fortuneteller.utils.SlidingWindow): Unit = {
    ...
    var totRes=window.calcTotal(0,1,currEval.a(0))/ tickSize;
    this.maxTicks = Math.max(this.maxTicks, totRes);
    this.minTicks = Math.min(this.minTicks, totRes);
  }
  override def finalizeCalibration(): List[CalibratedIndicator] = {
    ...
    cat.info(CalibratedIndicator(name, 1, "output.ticktotresult", maxTicks, minTicks, 1, -1))
    out.append(CalibratedIndicator(name, 1, "output.ticktotresult", maxTicks, minTicks, 1, -1))
    out.toList
  }
  override def processInput(window: fortuneteller.utils.SlidingWindow): Double = {
    val currEval = window.getElement(0)
    var totRes=window.calcTotal(0,1,currEval.a(0))/ tickSize;
    totRes = field.normalize(totRes);
    totRes
  }
