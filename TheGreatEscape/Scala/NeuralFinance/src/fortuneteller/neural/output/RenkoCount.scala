...
class RenkoCount(tickSize: Double, calibration: List[CalibratedIndicator]) extends OutputTarget with fortuneteller.indicators.IndicatorConfig {
  name = "RenkoCount"
  version="1"
  val cat = org.apache.log4j.Category.getInstance("fortuneteller.tickresult");
  val field = getNormalization(lookupCalibration("output.renkocount", calibration)); //new NormalizedField(NormalizationAction.Normalize,"diff",Config.getConfig.DONCHIAN_WIDTH_MAX,Config.getConfig.DONCHIAN_WIDTH_MIN,1,0);
  ...
  override def calibration(window: fortuneteller.utils.SlidingWindow): Unit = {
    var o =window.calcDirection(0,0)
  ...
  override def finalizeCalibration(): List[CalibratedIndicator] = {
    ...
    out.append(CalibratedIndicator(name, 1, "output.renkocount", maxTicks, minTicks, 1, -1))
    out.toList
  }
