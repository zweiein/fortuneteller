...
class BWFractals(inputWindow:Int,config:String,calibration:List[CalibratedIndicator]) extends NeuralIndicator with IndicatorConfig{
	name=getConfigField(config,"name")
	description=getConfigField(config,"description")
	version=getConfigField(config,"version")
	val cat = org.apache.log4j.Category.getInstance("fortuneteller.fractals");
	val field = getNormalization(lookupCalibration("fractals.value",calibration));//new NormalizedField(NormalizationAction.Normalize,"diff",Config.getConfig.FISHER_MAX,Config.getConfig.FISHER_MIN,1,-1);
	requestedValues.append(new RequestedValue(true,"bwFractals()[" + inputWindow + "]"))
	var minVal=1000.0
	var maxVal=(-1000.0)	
	...
	override def finalizeCalibration():List[CalibratedIndicator]={
	  ...
	  out.append(CalibratedIndicator(name, version.toInt,"fractals.value", maxVal, minVal,1,0))
	  out.toList
	}
